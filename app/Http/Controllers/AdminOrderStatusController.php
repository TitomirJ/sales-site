...
class AdminOrderStatusController extends Controller
{
        ...
        if (isset($request->confirm)){
            if(Auth::user()->isAdmin()){
                ...
                if($market_id == '1'){
                    ...
                    }elseif($request->delivery_method == 'УкрПочта'){

                        $code_up = str_replace(' ', '', $request->code_up);

                        $order->ttn = $code_up;

                        $text = 'Заказ отправлен УкрПочтой, код посылки: '.$code_up;
                        $response = RozetkaApi::putOrderStatus($order->order_market_id, 4, $text);
                        $message = 'Заказ №'.$order->id.' отправлен УкрПочтой, код посылки: '.$code_up;
                ...
                if($market_id == '1'){
                    ...
                    if($response->success){
                        ...
                        $data = [
                            ...
                            'msg' => $message,
                            'orderId' => $order->id,
                            'render' => view('admin.company.layouts.showCompany.dropdownStatusBtn', compact('order'))->render()
                        ];
                    }else{
                        $data = [
                            'status' => 'error',
        ...
        if (isset($request->confirm)){
            if(Auth::user()->isAdmin()){
                ...
                if($market_id == '1'){
                    ...
                    }elseif(in_array($order_rozet_status,$array_canceled_self)){// сиуация когда нужно отменить заказ который был отправлен новой почтой но покупатель отазался от товара
                        $data = self::cancelOrderTrait($order, $cancel_type, $cancel_text);
    ...
    private function contentModalConfirm(Request $request, $order){
        $action = $request->action;
        $render = view('admin.company.layouts.showCompany.layouts.orderStatusModalContent.confirm', compact('order', 'action'))->render();
        $data = [
            'status' => 'success',
            'action' => 'modal',
            'render' => $render,
    ...
    private function contentModalSend(Request $request, $order){
        $action = $request->action;
        $render = view('admin.company.layouts.showCompany.layouts.orderStatusModalContent.send', compact('order', 'action'))->render();
        $data = [
            'status' => 'success',
            'action' => 'modal',
            'render' => $render,
    ...
    private function contentModalReceived(Request $request, $order){
        $action = $request->action;
        $render = view('admin.company.layouts.showCompany.layouts.orderStatusModalContent.received', compact('order', 'action'))->render();
        $data = [
            'status' => 'success',
            'action' => 'modal',
            'render' => $render,
    ...
    private function contentModalCanceled(Request $request, $order){
        ...
        if($market_id == '1'){
            ...
            if($response->success){
                ...
                $render = view('admin.company.layouts.showCompany.layouts.orderStatusModalContent.canceled', compact('order', 'action', 'market_id', 'market_order_status', 'curent_st_roz'))->render();
            }else{
            ...
            $render = view('admin.company.layouts.showCompany.layouts.orderStatusModalContent.canceled', compact('order', 'action', 'market_id', 'curent_st_prom_and_other'))->render();
        }
        $data = [
            'status' => 'success',
            'action' => 'modal',
    ...
    /* Создание задачи на проверку статуса заказа */
        public function createJobCheckStatus(Request $request, $id){
            $order = Order::find($id);
            if($order){
                $response = ModuleJobs::createOrderCheckStatusJob($order->id, $order->marketplace_id, 'M', 0);
                return json_encode($response);
            }else{
                return json_encode([
                    'status' => 'error',
                    'msg' => 'Такого заказа нет в базе данных!'
                ]);
            }
        }
    /* Создание задачи на проверку статуса заказа (end)*/

	/**
     * изменение статуса заказа после проверки вручную отправляя запрос на розетку по Апи
     * 1.ответ от розетки и получение из ответа статус ,груп статус
     * 2.нахождение в БД локальной заказа и изменение его статуса в зависимости от полученных данных с розетки
     */
    public function checkStatusNow(Request $request, $id,$market)
    {

        $response = RozetkaApi::getDataOrder($market);
        ...
        if(!$response['success']){
           ...
           //данные из ответа по Апи от розетки на заказ
          $status_in_rozetka = $response['content']['status'];
          $status_group_rozetka = $response['content']['status_group'];
          //$ttn_rozetka = $response['content']['ttn'];

        // данные из таблицы своей по заказу
        $id_market = Order::where('order_market_id',$market)->get();

        foreach($id_market as $order){

               $status_local = $order->status;
               if($status_group_rozetka == '2' && $status_local !='1'){
                $order->status = '1';
                $order->save();
               }else if($status_group_rozetka == '3' && $status_local !='2'){
                $order->status = '2';
                $order->save();
				   self::changeTypeTrasactionAfterChangeStatusOrder($order->id);
        SF::recalculationBalanceCompany($order->company_id);
       ...
       return(json_encode($this->rozetka_status_orders[$status_in_rozetka]));
